...
public abstract class GenericDAO implements Serializable {
	...
	@SuppressWarnings("unchecked")
	public List<? extends IRecord> findAll(Class<? extends IRecord> recordClass, Criterion... criterion) {
		Session session = HibernateSessionManager.openSession();
	...
	public IRecord find(Class<? extends IRecord> recordClass, Integer IdNo) {
		Session session = HibernateSessionManager.openSession();
		IRecord record = (IRecord) session.createCriteria(recordClass).add(Restrictions.eq("idNo", IdNo))
				.uniqueResult();
		session.close();
		return record;
	}
